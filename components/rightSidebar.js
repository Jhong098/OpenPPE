...
export default function RightSideBar({toolbarOptions}) {
  ...
  return isOpen && (renderedComponent in whiteList) ? (
    <div class="relative">
      <button onClick={handleToolbarClose} className="fixed z-10 inset-0 h-full w-full bg-black opacity-25 cursor-default"></button>
      <div class="fixed z-20 bg-light_primary mt-navHeight p-6 w-64 top-0 right-0 bottom-0">
...
export function useToolbar(){
  ...
  const handleToolbarToggle = (componentName) => {
    if(renderedComponent !== componentName){ //not open or other component is open
      handleToolbarOpen(componentName)
    }  else if(isOpen) { //component is already on
      handleToolbarClose(componentName)
    }
  }
  
  return {isOpen, renderedComponent, handleToolbarClose, handleToolbarOpen, handleToolbarToggle}
}
